...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xt_ge, xt_meth, xt_mut, y, smile_graph, test_drug_dict):
        ...
        for i in range(data_len):
            ...
            # 从SMILES图结构中提取特征
            # smile_graph的形状是(231，6),{smile串:size,features,edge_index,coordinates,adjacency_matrix,distance_matrix}
            c_size, features, edge_index,coordinates = smile_graph[smiles][0:4]
...
def draw_pearson(pearsons, title):
    plt.figure()
    plt.plot(pearsons, label='val pearson')
